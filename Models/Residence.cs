...
namespace AirBB.Models
{
    public class Residence : IValidatableObject
    {
        ...
        [Range(1874, 2024, ErrorMessage = "Build year must be between 1874 and current year")]
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            if (BuildYear > currentYear)
            {
                yield return new ValidationResult(
                    ...
                    new[] { nameof(BuildYear) });
            }
            
            if (BuildYear < minimumYear)
            {
                yield return new ValidationResult(
                    $"Built year cannot be more than 150 years ago (minimum year: {minimumYear})",
                    new[] { nameof(BuildYear) });
            }
